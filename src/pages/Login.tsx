...
export function Login() {
  ...
  const { authenticate, signInWithOAuth, isAuthenticating, isOAuthSigningIn, authError, oauthError } = useAuth()
      ...
      { email, password },
      {
        onSuccess: (data) => {
          if (data.user && !data.error) {
            console.log('Authentication successful:', data.action)
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              {(error || authError || oauthError) && (
                <div className="text-sm text-red-600 text-center bg-red-50 p-3 rounded-lg border border-red-200">
                  {error || authError?.message || oauthError?.message}
                </div>
              )}
            ...
            <div className="mt-8">
              ...
              <div className="mt-6 grid grid-cols-2 gap-3">
                <Button
                  ...
                  disabled={isAuthenticating || isOAuthSigningIn}
                >
                  {/* Google SVG */}
